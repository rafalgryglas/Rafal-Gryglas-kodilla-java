...
public class Moves {
    ...
    public static MovesEnum computerCheatingMoves(MovesEnum playerMove) {
        Random computerRandomMoves = new Random();
        int computerMove = computerRandomMoves.nextInt(100) + 1; //70% to win by computer
        int computerElseMoves = computerRandomMoves.nextInt(3) + 1;//10% to DRAW and 20% to lose by computer
        if (playerMove == MovesEnum.ROCK) {
            if (computerMove >= 1 && computerMove < 36) {
                System.out.println("Computer chose SPOCK");
                return MovesEnum.SPOCK;
            } else if (computerMove >= 36 && computerMove < 71) {
                System.out.println("Computer chose PAPER");
                return MovesEnum.PAPER;
            } else {
                if (computerElseMoves == 1) {
                    System.out.println("Computer chose ROCK");
                    return MovesEnum.ROCK;
                } else if (computerElseMoves == 2) {
        ...
        } else if (playerMove == MovesEnum.PAPER) {
            if (computerMove >= 1 && computerMove < 36) {
                System.out.println("Computer chose SCISSORS");
                return MovesEnum.SCISSORS;
            } else if (computerMove >= 36 && computerMove < 71) {
        ...
        } else if (playerMove == MovesEnum.LIZARD) {
            if (computerMove >= 1 && computerMove < 36) {
                System.out.println("Computer chose ROCK");
                return MovesEnum.ROCK;
            } else if (computerMove >= 36 && computerMove < 71) {
        ...
        } else if (playerMove == MovesEnum.SPOCK) {
            if (computerMove >= 1 && computerMove < 36) {
                System.out.println("Computer chose LIZARD");
                return MovesEnum.LIZARD;
            } else if (computerMove >= 36 && computerMove < 71) {
    ...
    public static void selectGamesMoves(String name, int rounds) {
        ...
        while (roundNumber < rounds) {
            ...
            MovesEnum computerMove = computerCheatingMoves(playerMove);
